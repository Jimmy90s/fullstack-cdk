...
export class AwsCognitoStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: IAwsCognitoStackProps) {
    ...
    const table = ddb.Table.fromTableArn(
      this,
      "AccountTable",
    ...
    const googleLogin = new cognito.UserPoolIdentityProviderGoogle(
      this,
      "GoogleLogin",
      {
        userPool,
    ...
    /* FACEBOOK LOGIN */
    const facebookSecret = secretsmanager.Secret.fromSecretNameV2(
      this,
      "FacebookClientSecret",
      props.imports.facebookSecretName
    );
    const facebookLogin = new cognito.UserPoolIdentityProviderFacebook(
      this,
      "FacebookLogin",
      {
        userPool,
        clientId: facebookSecret
          .secretValueFromJson("id")
          .unsafeUnwrap()
          .toString(),
    ...
    const authenticatedUserRole = new iam.Role(
      this,
    ...
    const bucketActionsPolicy = new iam.PolicyStatement({
      ...
      resources: [
        bucket.arnForObjects("users/${cognito-identity.amazonaws.com:sub}/*"),
      ],
      effect: iam.Effect.ALLOW,
    });
